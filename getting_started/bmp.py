...
def write_grayscale(filename, pixels):
    ...
    Args:
        ...
        Each rows must be an iterable series of integers in range 0-255
    ...
    with open(filename, 'wb') as bmp:
        ...
        bmp.write(b'\x00\x00') # #Unused 116-bite integer - should be zero
        bmp.write(b'\x00\x00') # #Unused 116-bite integer - should be zero

        pixel_offset_bookmark = bmp.tell() # The next four bytes should hold the integer offset to the pixel data, Zero placeholder for now.
        ...
        bmp.write(b'\x01\x00') # Number of image plans
        bmp.write(b'\x00\x00') # Bits per pixel 0 for grayscale
        ...
        for c in range(256):
            bmp.write(bytes((c,c,c,0))) # Blue, Green ,Read, Zero
